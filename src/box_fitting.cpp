...
//static std::string filename="/home/eaksu3/Datasets/TM/Tm.csv";
    bool first=true;
char textBuffer_point[20];
char textBuffer_clusterid[20];
// float picScale = 30;
...
int lnumPoints = 300;
ros::Time now;
float sensorHeight = 1.43;
// float tHeightMin = 1.2;
float tHeightMin = 0.7;
float tHeightMax = 5;
// float tWidthMin = 0.5;
// float tWidthMin = 0.4;
...
float tLenMax = 20.0;
float tAreaMax = 40.0;
float tRatioMin = 1.0; //1.3;
float tRatioMax = 15.0; //5
float minLenRatio = 1.0;
float tPtPerM3 = 8;
...
bool ruleBasedFilter(vector<Point2f> pcPoints, float maxZ, int numPoints , ros::Publisher pointcloudPub, ros::Publisher clusteridPub, string frame, visualization_msgs::MarkerArray &pointnumber_marker, visualization_msgs::MarkerArray &clusterid_marker, int i){
    ...
    if(numPoints < 3) return isPromising;
    ...
    mass = area*height;
    ratio = length/width;
    cout<< "area " <<area <<" height:  " <<height<< " mass: " <<mass << " length: "<< length << " ratio: " <<ratio<< endl;
    cout<< "width " <<width << "numpoints: "<< numPoints<<" position: "<< x1 << " "<<x2<<" "<<x3 <<endl;
    ...
    if(height > tHeightMin && height < tHeightMax){
        if(width > tWidthMin && width < tWidthMax){
            if(length > tLenMin && length < tLenMax){
                if(area < tAreaMax){
                    if(numPoints > area*tPtPerM3){
                        if(length > minLenRatio){
                            if(ratio > tRatioMin && ratio < tRatioMax){
                                isPromising = true;
                                cout<< " ispromising : " << isPromising << endl;

                               // ofs << " ispromising : "<< ","
                                 //   << isPromising << std::endl;
                                return isPromising;
                            }
                        }
                        else{
...
void getBoundingBox(vector<PointCloud<PointXYZ>>  clusteredPoints,
                    vector<PointCloud<PointXYZ>>& bbPoints ,ros::Publisher pointcloudPub, ros::Publisher clusteridPub, string frame, visualization_msgs::MarkerArray pointnumber_marker, visualization_msgs::MarkerArray clusterid_marker){
    pointnumber_marker.markers.resize(clusteredPoints.size());
    clusterid_marker.markers.resize(clusteredPoints.size());
    for (int iCluster = 0; iCluster < clusteredPoints.size(); iCluster++){
        ...
        if(slopeDist > lSlopeDist && numPoints > lnumPoints){
            ...
            bool isPromising = ruleBasedFilter(pcPoints, maxZ, numPoints ,pointcloudPub, clusteridPub, frame, pointnumber_marker, clusterid_marker, iCluster);
...
//        clustered2D[iCluster] = m;
 ...
 pointcloudPub.publish(pointnumber_marker);
 clusteridPub.publish(clusterid_marker);
}
...
vector<PointCloud<PointXYZ>> boxFitting(PointCloud<PointXYZ>::Ptr elevatedCloud,
 ...
 //fs.open(filename.c_str(), std::ios_base::app);
  
    vector<PointCloud<PointXYZ>>  clusteredPoints(numCluster);
    getClusteredPoints(elevatedCloud, cartesianData, clusteredPoints);

    //ofs<<"number of cluster" << "," << numCluster << std::endl;
    vector<PointCloud<PointXYZ>>  bbPoints;
    getBoundingBox(clusteredPoints, bbPoints, pointcloudPub, clusteridPub, frame , pointnumber_marker, clusterid_marker);
